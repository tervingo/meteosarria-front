...
const AemetForecastTable = ({ city, styles, isMobile }) => {
  const [data, setData] = useState(null);
  const [error, setError] = useState(null);

  useEffect(() => {
    let isMounted = true;
    axios.get(`${BACKEND_URI}/api/aemet-forecast/${city}`)
      .then((response) => {
  ...
  if (error) {
    return (
      <Typography style={{ ...styles.resumen, marginTop: '20px' }}>
        No se ha podido cargar la previsión de AEMET.
      </Typography>
    );
  }
...
export default AemetForecastTable;
